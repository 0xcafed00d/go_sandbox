...
struct EliteCore::Impl
{
	...
	Impl(size_t galaxyNumber);
	~Impl();

	SystemInfo MakeSystem (PlanetSeed_t& seed);
...
const EliteCore::SystemInfo& EliteCore::GetSystemInfo(size_t systemIndex) const
{
    return m_impl->m_galaxy[systemIndex];
}

std::string EliteCore::GetEconomyName (unsigned int economy)
{
    if (economy >= 8)
...
std::string EliteCore::GetGovernmentName (unsigned int govtype)
{
    if (govtype >= 8)
...
size_t EliteCore::FindSystemByName (const std::string& name) const
{
...
float EliteCore::DistanceBetweenSystems (size_t system1, size_t system2) const
{
...
void EliteCore::FindSystemsWithin (size_t systemFrom, float radius_LY, std::vector<size_t>& result ) const
{
	result.clear();
...
size_t EliteCore::GetClosestSystem (unsigned int x, unsigned int y) const
{
